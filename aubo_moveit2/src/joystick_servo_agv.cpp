...
const char BASE_FRAME_ID[] = "aubo_base";
const float scale_joint = 2.5;
const float scale_twist = 1.0; //must <= 1  0.806

const int MODBUS_SIGNAL_GRIPPER = 0;
// Enums for button names -> axis/button array index
// For XBOX one plus controller
// enum Axis
// {
//   LEFT_STICK_X = 0,
//   LEFT_STICK_Y = 1,
  ...
  std::unique_ptr<control_msgs::msg::JointJog> & joint)
{
  ...
  if (buttons[A] || buttons[B] || buttons[X] || buttons[Y] || axes[D_PAD_X] || axes[D_PAD_Y]) {
    ...
    joint->velocities.push_back(0.8 * scale_joint * axes[D_PAD_Y]);
  ...
  twist->twist.linear.y = -1.0 * scale_twist * axes[RIGHT_STICK_Y];
  twist->twist.linear.x = 1.0 * scale_twist * axes[RIGHT_STICK_X];

  double lin_x_right = -0.5 * (axes[RIGHT_TRIGGER] - AXIS_DEFAULTS.at(RIGHT_TRIGGER));
  double lin_x_left = 0.5 * (axes[LEFT_TRIGGER] - AXIS_DEFAULTS.at(LEFT_TRIGGER));
  twist->twist.linear.z = 1.0 * scale_twist * (lin_x_right + lin_x_left);

  twist->twist.angular.x =  scale_twist * axes[LEFT_STICK_Y];
  twist->twist.angular.y =  scale_twist * axes[LEFT_STICK_X];
...
public:
  ...
  : Node("joy_to_twist_publisher", options), frame_to_publish_(BASE_FRAME_ID)
  {
    ...
    modbus_set_signal_client_ = this->create_client<aubo_msgs::srv::SetOutputSignal>("/io_and_status_controller/modbus_set_output_signal");
    modbus_set_signal_client_->wait_for_service(std::chrono::seconds(1));
  ...
  void joyCB(const sensor_msgs::msg::Joy::SharedPtr msg)
  {
    ...
    auto modbus_request = std::make_shared<aubo_msgs::srv::SetOutputSignal::Request>();
    modbus_request->signal_name_index = MODBUS_SIGNAL_GRIPPER;
    if(msg->buttons[LEFT_STICK_CLICK]){
      modbus_request->value = 0;
      modbus_set_signal_client_->async_send_request(modbus_request);
    }
    if(msg->buttons[RIGHT_STICK_CLICK]){
      modbus_request->value = 1000;
      modbus_set_signal_client_->async_send_request(modbus_request);
    }
    // if(msg->buttons[MENU]){
...
private:
  ...
  rclcpp::Client<aubo_msgs::srv::SetOutputSignal>::SharedPtr modbus_set_signal_client_;
